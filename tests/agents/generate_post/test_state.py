...
def test_initialization():
    state = GeneratePostState(
        ...
        pageContents=["content1", "content2"],
    ...
    assert state.pageContents == ["content1", "content2"]
...
def test_increment_condense_count(state):
    state.condenseCount += 1
    assert state.condenseCount == 1