...
from backend.retrieval_graph.writer_graph.state import QueryState, WriterState
...
async def generate_queries(
    ...
    messages = [
        {"role": "system", "content": configuration.generate_queries_system_prompt.format(
            max_count=configuration.generate_queries_count)},
...
async def write(
    state: WriterState, *, config: RunnableConfig
) -> dict[str, list[Document]]:
    """Create writeup based on user inputs, reference documents, additional context, and editor feedback.

    Args:
        state (WriterState): The current state containing messages, reference documents, and additional context.
        config (RunnableConfig): Configuration with the retriever used to fetch documents.
